...
- Use one build-time route/data snapshot for runtime-discovered routes. `getStaticPaths()` and hydrated React islands must share the same upcoming-call and active-network snapshots.
  - Snapshots (`src/data/generated/{upcoming-calls,devnet-networks}.json`) are committed to git. The normal `dev`/`build` commands read them as-is; `build:fresh` (used by `predeploy` and the deploy workflow) re-runs `snapshot-routes` to refresh them from the live GitHub issues and cartographoor endpoints first.
  - Snapshot fetches are bounded-retried (transient 5xx / network blips). In CI, an *unrecovered* fetch failure is fail-closed: `snapshot-routes` aborts the build rather than ship stale or empty route data, even when a valid snapshot is already committed (`FAIL_ON_STALE = CI && !ALLOW_STALE_ROUTE_SNAPSHOTS`). This guards both upstreams. A non-recovered GitHub failure — including a 403/429 rate-limit, not just a 5xx — is fail-closed too (which is why `deploy.yml` passes `GITHUB_TOKEN` to raise the limit), and cartographoor is unauthenticated, so a sustained outage there can block a deploy. Recovery lever for an on-call maintainer: re-run with `ALLOW_STALE_ROUTE_SNAPSHOTS=1` to accept the committed snapshot. Local/offline builds are already lenient (they keep the previous snapshot).
