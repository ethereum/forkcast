...
export default function GlobalCallSearch({
  isOpen,
  onClose,
  initialQuery = "",
}: GlobalCallSearchProps) {
  ...
  const performSearch = useMemo(
    () =>
      debounce(
        async (searchQuery: string, contentType: string, callType: string) => {
          ...
          try {
            ...
            const formattedResults: GlobalSearchResult[] = searchResults.map(
              (result) => ({
          ...
          {query && results.length === 0 && !loading && !indexBuilding ? (
            ...
            <div className="p-8 text-center text-slate-400 dark:text-slate-500">
              <p className="text-sm mb-3">
                Start typing to search across all calls
              </p>
        ...
        {query && results.length > 0 && (
          <div className="border-t border-slate-200 dark:border-slate-700 px-3 sm:px-4 py-2 flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
            <div className="flex items-center gap-2 sm:gap-4">
              <span>
                {searchStats?.shown || results.length} result
                {(searchStats?.shown || results.length) !== 1 ? "s" : ""}
              </span>
