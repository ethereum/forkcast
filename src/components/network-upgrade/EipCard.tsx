...
export const EipCard: React.FC<EipCardProps> = ({
  ...
  const {
    data: pokebalData,
    loading: pokebalLoading,
    error: pokebalError,
  } = usePokebalData(eip.id);

  return (
    <article
      key={eip.id}
      className={`bg-white dark:bg-slate-800 border rounded p-8 ${
    ...
    >
            ...
            <div className="flex items-center gap-3 group relative">
              ...
              <h3 className="text-xl font-medium text-slate-900 dark:text-slate-100 leading-tight flex-1">
                {isHeadliner(eip, forkName) && (
                  ...
                  >
                    <span className="text-purple-400 hover:text-purple-600 dark:text-purple-500 dark:hover:text-purple-400 mr-2 transition-colors cursor-help">
                      {(() => {
                        const inclusionStage = getInclusionStage(eip, forkName);
                        const isSFI =
                          inclusionStage === "Scheduled for Inclusion";
                ...
                <span className="text-slate-400 dark:text-slate-500 text-sm font-mono mr-2 relative -top-px">
                ...
                {layer && (
                  <Tooltip
                    text={
                      layer === "EL"
                        ? "Primarily impacts Execution Layer"
                        : "Primarily impacts Consensus Layer"
                    }
                    className="inline-block"
                  >
                    <span
                      className={`px-2 py-1 text-xs font-medium rounded ml-2 relative -top-px ${
                        layer === "EL"
                ...
                {champion ? (
                  <>
                    <div className="inline-flex items-center gap-2">
                      ...
                      <button
                        onClick={() =>
                          setShowChampionDetails(!showChampionDetails)
                        }
                  ...
                  <div className="inline-flex items-center gap-2 bg-amber-50/80 dark:bg-amber-900/10 border border-amber-200/60 dark:border-amber-700/30 rounded px-2.5 py-1.5">
                    ...
                    <span className="text-xs text-amber-700 dark:text-amber-400/90 italic">
                      Not yet assigned
                    </span>
                  </div>
                )}
      ...
      <div className="mt-8 space-y-8">
            ...
            <ul className="space-y-2">
              {eip.benefits.map((benefit, index) => (
                <li key={index} className="flex items-start text-sm">
                  <span className="text-emerald-600 dark:text-emerald-400 mr-3 mt-0.5 text-xs">
                    ●
                  </span>
        ...
        {eip.stakeholderImpacts &&
          Object.keys(eip.stakeholderImpacts).length > 0 && (
            <div>
              <h4 className="text-sm font-semibold text-slate-900 dark:text-slate-100 mb-4 uppercase tracking-wide">
                Stakeholder Impact
              </h4>
              <div className="bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded p-4">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                  {Object.entries(eip.stakeholderImpacts).map(
                    ([stakeholder, impact]) => {
                      ...
                      return (
                        ...
                        >
                          <h5 className="font-semibold text-slate-900 dark:text-slate-100 text-xs mb-3 border-b border-slate-100 dark:border-slate-700 pb-2">
                            {
                              stakeholderNames[
                                stakeholder as keyof typeof stakeholderNames
                              ]
                            }
                          </h5>
          ...
          <div>
            ...
            <ul className="space-y-2">
              {eip.tradeoffs.map((tradeoff, index) => (
                <li key={index} className="flex items-start text-sm">
                  <span className="text-amber-600 dark:text-amber-400 mr-3 mt-0.5 text-xs">
                    ⚠
                  </span>
                  <span className="text-slate-700 dark:text-slate-300">
                    {tradeoff}
              ...
              <div className="space-y-4">
                {eip.northStarAlignment?.scaleL1 && (
                  <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 rounded p-4">
                    <h5 className="font-semibold text-slate-900 dark:text-slate-100 text-xs mb-3 border-b border-blue-200 dark:border-blue-600 pb-2">
                      Scale L1
                    </h5>
                    <p className="text-slate-700 dark:text-slate-300 text-xs leading-relaxed">
                      {eip.northStarAlignment.scaleL1.description}
                    </p>
                  </div>
                )}
                {eip.northStarAlignment?.scaleBlobs && (
                  <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 rounded p-4">
                    <h5 className="font-semibold text-slate-900 dark:text-slate-100 text-xs mb-3 border-b border-purple-200 dark:border-purple-600 pb-2">
                      Scale Blobs
                    </h5>
                    <p className="text-slate-700 dark:text-slate-300 text-xs leading-relaxed">
                      {eip.northStarAlignment.scaleBlobs.description}
                    </p>
                  </div>
                )}
                {eip.northStarAlignment?.improveUX && (
                  <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 rounded p-4">
                    <h5 className="font-semibold text-slate-900 dark:text-slate-100 text-xs mb-3 border-b border-emerald-200 dark:border-emerald-600 pb-2">
                      Improve UX
                    </h5>
                    <p className="text-slate-700 dark:text-slate-300 text-xs leading-relaxed">
                      {eip.northStarAlignment.improveUX.description}
                    </p>
                  </div>
                )}
              </div>
