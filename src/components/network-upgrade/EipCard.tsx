...
interface EipCardProps {
  ...
  /** Opens the EIP in a drawer instead of navigating; omit to keep plain links. */
...
export const EipCard: React.FC<EipCardProps> = ({
  ...
  // Anywhere on the card opens the drawer, so the card's own links, buttons and the
  // tail of a drag-selection have to keep their click rather than surrender it.
  ...
  return (
    <article
      key={eip.id}
      className={`bg-white dark:bg-slate-800 border rounded p-6 transition-colors ${
      ...
      } ${onOpenDrawer ? 'cursor-pointer hover:border-purple-300 dark:hover:border-purple-500' : ''}`}
      id={eipId}
      data-section
      onClick={openFromCard}
    >
      {/* Header */}
      <header className="mb-2 group relative">
      ...
      <div>
        ...
        {(() => {
          ...
          const hasResources = eip.supportingDocuments && eip.supportingDocuments.length > 0;
