...
const EditableDateCell: React.FC<EditableDateCellProps> = ({
  ...
  if (!isEditable) {
    ...
    return (
      <div className="flex items-center gap-1">
        ...
        <span className={`text-xs w-10 text-right ${gapIsNegative ? 'text-red-600 dark:text-red-400 font-semibold' : gapIsWarning ? 'text-amber-600 dark:text-amber-400 font-medium' : 'text-slate-400 dark:text-slate-500'}`}>
          {gapText || ''}
        </span>
      </div>
    );
    ...
    return (
      <div className="flex items-center gap-1">
        <input
          ...
          autoFocus
          className="px-1 py-0.5 text-sm border border-purple-400 dark:border-purple-500 rounded bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-1 focus:ring-purple-500 w-32"
        />
      </div>
    );
  }
  ...
  return (
    <div className="flex items-center gap-1 group">
      ...
      <div
        className={`text-sm cursor-pointer ${isOverdue ? 'text-red-600 dark:text-red-400 font-semibold hover:text-red-700 dark:hover:text-red-300' : 'text-slate-700 dark:text-slate-300 hover:text-purple-600 dark:hover:text-purple-400'}`}
      ...
      <button
        ...
        className={`text-xs transition-opacity ${isLocked ? 'text-amber-500' : 'opacity-0 group-hover:opacity-100 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300'}`}
